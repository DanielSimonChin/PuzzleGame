...
function PlayerManager() {
  ...
  function storeGameStats(theStatus) {
    ...
    var dim =
      document.getElementById("dimensionList").options[
        document.getElementById("dimensionList").selectedIndex
      ].value;
...
function Utility() {
  ...
  function checkFormFilled() {
    ...
    if (
      ...
      dimSize.options[dimSize.selectedIndex].value == 0 ||
      chrono != undefined
    ) {
  ...
  function showStats() {
    ...
    var heading =
      "<th>#</th><th>Name</th><th>Dimension</th><th>Moves</th><th>Duration</th><th>Status</th>";
    var content = "";
    for (var i = 1; i < playerManager.listPlayerArr.length + 1; i++) {
      ...
      content =
        ...
        "</td>" +
        "<td>" +
        playerManager.listPlayerArr[i - 1].name +
        "</td>" +
        "<td>" +
        playerManager.listPlayerArr[i - 1].boardDimension +
        "</td>" +
        "<td>" +
        playerManager.listPlayerArr[i - 1].numMoves +
        "</td>" +
        "<td>" +
        playerManager.listPlayerArr[i - 1].duration +
        "</td>" +
        "<td>" +
        playerManager.listPlayerArr[i - 1].theStatus +
        "</td>" +
        "</tr>";
    }

    var tableEnd = "</table>";
    document.getElementById("leaderboardsTable").innerHTML =
      tableBegin + heading + content + tableEnd;
  }
}
  ...
  for (var i = 0; i < puzzleGameObj.puzzleWidth; i++) {
    ...
    for (var j = 0; j < puzzleGameObj.puzzleWidth; j++) {
      ...
      if (puzzleGameObj.puzzleBoard[i][j].indexNumber == 0) {
        ...
        htmlRows +=
          "<td class='filledType'>" +
          puzzleGameObj.puzzleBoard[i][j].indexNumber +
          "</button>" +
          "</td>";
      }
    }
...
function swap2Tiles(indexTile1, indexTile2) {
  ...
  var tempIndexNumber =
    puzzleGameObj.puzzleBoard[indexTile1[0]][indexTile1[1]].indexNumber;

  puzzleGameObj.puzzleBoard[indexTile1[0]][indexTile1[1]].indexNumber =
    neighbour.indexNumber;
  puzzleGameObj.puzzleBoard[indexTile1[0]][indexTile1[1]].tileType =
    "emptyTile";

  puzzleGameObj.puzzleBoard[indexTile2[0]][indexTile2[1]].indexNumber =
    tempIndexNumber;
  puzzleGameObj.puzzleBoard[indexTile2[0]][indexTile2[1]].tileType =
    "filledType";
}
//works, tested in console
...
function getNeighboursIndicesArr(position) {
  ...
  if (
    north[0] < 0 ||
    north[1] < 0 ||
    north[0] >= puzzleGameObj.puzzleWidth ||
    north[1] >= puzzleGameObj.puzzleWidth
  ) {
    returnValues[0] = -1;
  } else {
  ...
  if (
    east[0] < 0 ||
    east[1] < 0 ||
    east[0] >= puzzleGameObj.puzzleWidth ||
    east[1] >= puzzleGameObj.puzzleWidth
  ) {
    returnValues[1] = -1;
  } else {
  ...
  if (
    south[0] < 0 ||
    south[1] < 0 ||
    south[0] >= puzzleGameObj.puzzleWidth ||
    south[1] >= puzzleGameObj.puzzleWidth
  ) {
    returnValues[2] = -1;
  } else {
  ...
  if (
    west[0] < 0 ||
    west[1] < 0 ||
    west[0] >= puzzleGameObj.puzzleWidth ||
    west[1] >= puzzleGameObj.puzzleWidth
  ) {
    returnValues[3] = -1;
  } else {
...
function processClickTile(indexValue) {
  ...
  if (indexMoveTo != null) {
    ...
    setupTileClickEvent();
    if (
      match2States(puzzleGameObj.puzzleBoard, puzzleGameObj.goalState) == true
    ) {
      gameWon = true;
      cancelGame();
      utility.playAudio();
...
function init() {
  ...
  document
    ...
    .addEventListener("input", utility.checkFormFilled);
  document
    .getElementById("dimensionList")
    .addEventListener("input", utility.checkFormFilled);
